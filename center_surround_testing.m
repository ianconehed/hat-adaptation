...
k_adap = .5;
tau_adap = 400;
corr_mat = zeros(10,t_total);
figure('Renderer', 'painters', 'Position', [200 300 600 700])
subplot(2,1,1)
for trial = 1:40
    rng(trial);
    ...
    alpha = 1.3; %.3, .9
    sigma = .6; %1.2, .7%weird behavior under one
    alpha_in = 1;
    sigma_in = 4;
    beta = -13.5;
    ind = 1:N;
...
%             PI_it_x(:,1:100) = .2*ones(N,100).*normrnd(0,1,N,1);
        elseif k == 2
    %         PI_it_t = PI_it_t1;
    %         PI_it_t(:,1:100) = 1*ones(N,100).*normrnd(0,1,N,1);
            PI_it_x(:,1501:1600) = 1*ones(N,100).*I_ij(:,randi(N));
...
final = nonzeros(corr_mat(:,1500));
final_tally = sum(final>.8)/length(final);

disp(final_tally)

final_2 = nonzeros(corr_mat(:,t_total-1));
final_tally2 = sum(abs(final_2)<.4)/length(final_2);
...
% figure
% mean_v = mean(corr_mat,1);
% std_v = std(corr_mat,1);
% curve1 = mean_v + std_v;
% curve2 = mean_v - std_v;
% x_1 = 1:t_total;
% x_2 = [x_1, fliplr(x_1)];
% inBetween = [curve1, fliplr(curve2)];
% fill(x_2, inBetween, 'g');
% hold on;
% plot(x_1, mean_v, 'r', 'LineWidth', 2);
