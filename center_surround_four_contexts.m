...
I_ij = mpa*sincy(a*dist).*(abs(dist)<=pi/a) ...
    + spa*sincy((dist + ((k-1)*pi/a))/(k/a)).*(sincy((dist + ((k-1)*pi/a))/(k/a))>0).*(dist> pi/a)...
    + spa_n*sincy((dist + ((k-1)*pi/a))/(k/a)).*(sincy((dist + ((k-1)*pi/a))/(k/a))<0).*(dist> pi/a)...
    + spa*sincy((dist - ((k-1)*pi/a))/(k/a)).*(sincy((dist - ((k-1)*pi/a))/(k/a))>0).*(dist< pi/a)...
    + spa_n*sincy((dist - ((k-1)*pi/a))/(k/a)).*(sincy((dist - ((k-1)*pi/a))/(k/a))<0).*(dist< pi/a);
...
%         PI_it_x(:,1101:1200) = 1*ones(N,100).*I_ij(:,randi(N));
        PI_it_y(:,1:4000) = .5*ones(N,4000).*I_ij(:,N/2 - sqrt(N)/2);
%         PI_it_y(:,1:4000) = 1*ones(N,4000).*normrnd(0,1,N,1);
    elseif k == 2
        PI_it_y(:,1:4000) = .5*ones(N,4000).*I_ij(:,2200 - sqrt(N)/2);
%         PI_it_y(:,1:4000) = 1*ones(N,4000).*normrnd(0,1,N,1);
%         PI_it_t = PI_it_t1;
